...
- **Anti-flicker / zero-render hash short-circuit**: every sync round-trip is gated by a **FNV-1a 32-bit hash** of the SRS store + theme (`src/utils/syncHash.js`). If the local hash matches the remote hash (pull) or the `lastSyncedHashRef` (push/mutation watcher/safety-net), the operation short-circuits as a no-op — no `replaceStore`, no `setTheme`, no revision bump, no debounce timer, no network call. The React tree doesn't re-render. The user perceives zero flicker. `replaceStore` itself hash-checks inside `useSRS`, so even if the engine's check missed something, the SRS layer is idempotent against no-op merges. The old `skipNextPushRef` escape hatch is GONE — the hash check makes it unnecessary.
...
- `src/utils/syncMerge.js`: pure LWW merge. Exports `mergeSRSStores(local, remote)`, `shouldPushLocal(local, remote)`, and `compareStatesByHash({localStore, localTheme, remoteStore, remoteTheme})` (returns `{hasChanges, mergedStore, hash}`; short-circuits to `hasChanges: false` when local+remote hash identically). Compares cards by `srs.lastReviewed` (fallback `srs.nextReview` → `card.createdAt`), preserves winner's structural fields (`type/verbKey/front/infinitivo`), `order` is union with each side's relative order preserved, tie-breaker = remote.
- `src/utils/syncHash.js`: lightweight FNV-1a 32-bit hash utility. Exports `calculateStoreHash(srsStore, theme)` (the primary engine comparison key), `calculateCardHash(card)` (strips the per-device `id` field), and `readThemeFromLocalStorage()` (shared with `useSyncEngine`). Uses a custom key-sorted `stableStringify` so two objects with the same keys in different insertion orders hash identically. ~0.44 ms per hash for a realistic 100-card deck. Zero deps.
...
## Build
`pnpm build` → `dist/` produces **~376 KB JS raw / ~117 KB gzipped** (1821 modules after silent-sync engine + hash optimization) and **~134 KB CSS raw / ~21 KB gzipped**. MongoDB driver NOT in client bundle; lucide-react IS in the client bundle (tree-shaken, ~40 unique icons). The sync engine (utils + hook + context + 2 components) added ~20 KB raw JS and ~8 KB CSS (DaisyUI `modal`/`dialog`/`btn-circle` primitives + the new Cloud icon). The hash utility added ~1.3 KB raw JS (`stableStringify` + FNV-1a + `calculateStoreHash` + `calculateCardHash`). Build cost over time:
...
- 375 KB → 376 KB JS / +0.07 KB CSS after hash short-circuit (`syncHash.js` + hash checks in `useSRS.replaceStore` + `lastSyncedHashRef` in engine). Net +1.3 KB raw JS for the hash utility; hash-call sites inlined into existing functions, no new components.
...
- **Hash short-circuit verified in Node**: `calculateStoreHash` is order-insensitive (cards in different insertion orders hash identically) and content-sensitive (different `srs.lastReviewed` produces different hashes). 100 hashes of a 100-card deck run in ~44 ms total (~0.44 ms each). Card-level hash strips the per-device `id` so two devices with locally-generated ids hash the same content identically.

### Anti-flicker hash design (`src/utils/syncHash.js`)

The most common case during sync is a **no-op round-trip**: the user opened the app, the engine pulled remote, and remote matches local exactly. Without optimization, this would still:
- call `srs.replaceStore(merged)` → `setStore` → re-render the whole SRS tree → flicker
- bump `revision` → trigger the mutation watcher → schedule a 2 s debounce → push the same state back to Atlas → network round-trip

The hash short-circuit kills both:

1. **Engine pull**: `compareStatesByHash` runs FNV-1a on `(localStore, localTheme)` and `(remoteStore, remoteTheme)`. If they match, returns `{hasChanges: false, mergedStore: localStore, hash: localHash}` and `applyRemote` records the hash in `lastSyncedHashRef` without touching `srs.replaceStore` or `themeApi.setTheme`. Result: zero React updates, zero re-renders, zero network.

2. **SRS layer** (`useSRS.replaceStore`): computes the incoming hash and the current hash (with the same theme read from localStorage). If they match, returns `false` and skips `setStore` + `writeStore` + `setRevision`. Belt-and-suspenders: even if the engine miscalculates something, the SRS layer is idempotent against a no-op merge.

3. **Mutation watcher** (the `useEffect` watching `srsRevision` + `currentTheme`): hashes current local state and compares to `lastSyncedHashRef`. If equal, returns immediately without scheduling a debounce timer. Catches StrictMode double-invoke, hot-reload, parent re-renders that don't actually change the SRS.

4. **`pushCurrent`**: hashes the payload it's about to send and compares to `lastSyncedHashRef`. If equal, marks `pendingPush = false`, `status = 'synced'`, and returns. Catches repeated calls from the pagehide flush + 5-min safety-net + manual `forcePushNow` when nothing has changed.

5. **`pagehide` flush + 5-min safety-net**: both call `pushCurrent` (or a thin wrapper that hashes first). If nothing's changed, no network call. The safety-net becomes a free no-op instead of a periodic waste of bandwidth.

#### `lastSyncedHashRef` lifecycle

- **Initialized to `null`** on mount. The first mutation watcher fire sees `currentHash !== null` and schedules the bootstrap push. The bootstrap pull runs in parallel and either:
  - finds remote matching local → `applyRemote` sets `lastSyncedHashRef = remoteHash` → the subsequent watcher fire (if any) sees equal hashes and skips the push;
  - finds remote empty + local empty → `lastSyncedHashRef = localHash`;
  - finds remote differs → `applyRemote` runs the LWW merge and bumps `revision` → the watcher fires with a new hash and pushes the merged state back to Atlas (so Atlas has the converged state).
- **Updated after every successful push**: `lastSyncedHashRef.current = hashOf(whatWeJustPushed)`.
- **Updated after a no-op pull**: `lastSyncedHashRef.current = remoteHash` (which equals localHash). We know Atlas has this state.
- **Updated on `linkNewToken` / `unlink`**: reset to `null` so the new token starts with a clean slate.
- **NOT updated after a failed push**: we want the next watcher / safety-net to retry.

#### Why FNV-1a 32-bit

- Single-pass, no allocations, ~10 ns per byte. ~44 ms for 100 hashes of a 100-card deck on a 2020-era CPU.
- 8-char hex output is small enough to store in a `useRef` cheaply.
- Non-cryptographic but uniform enough for change detection. Collisions would only cause a missed update — both sides compute the same hash, so collisions can only suppress a real push, never corrupt data.
- Zero deps. The whole utility is ~150 lines, no external packages.

#### What the hash includes

- **Cards**: each card's content (type, verbKey, front, infinitivo, createdAt, srs.{interval, ef, repetitions, lastReviewed, nextReview}). The per-device-generated `id` is STRIPPED — cards are matched by content identity, not local-storage key.
- **Order array**: comma-joined id list. Two decks with the same cards in different insertion orders hash differently (this is correct: reordering the deck is a user-visible change).
- **Theme**: hashed separately and concatenated into the final hash. Theme-only changes are detected.

#### What the hash does NOT include

- The card `id` field (per-device-generated).
- The syncToken (different per device by definition).
- `lastActiveAt` from Atlas (server-managed).
- Any UI-only state (current cursor, form input, etc.).

#### Performance budget

- Hash computation: ~0.44 ms for a 100-card deck, dominated by string serialization.
- Three places call `calculateStoreHash` per sync round-trip (pull, push, watcher) = ~1.3 ms total. Negligible compared to the network round-trip (~50–200 ms).
...
```
(this session — hash optimization)
[pending] docs(handoff): record sync hash optimization (d7b9b91)
d7b9b91 feat(sync): hash-based anti-flicker short-circuit for sync engine
  Anti-flicker / zero-render design: every sync round-trip is gated
  by a FNV-1a 32-bit hash of the SRS store + theme.

  New utility
  - src/utils/syncHash.js: calculateStoreHash(srsStore, theme) returns
    a deterministic 8-char hex string. Uses stableStringify (alphabetical
    key sort, NaN/Infinity normalization) so two stores with the same
    content but different insertion orders hash identically. Strips the
    per-device card `id` before hashing (cards are matched by content
    identity, not by local-storage key).
  - ~0.44 ms per hash for a realistic 100-card deck. Zero deps.

  Merge layer (src/utils/syncMerge.js)
  - New compareStatesByHash({localStore, localTheme, remoteStore,
    remoteTheme}) returns {hasChanges, mergedStore, hash}.
    Short-circuits to hasChanges:false when local+remote hash identically.
  - mergeSRSStores() preserved as a lower-level primitive; engine
    doesn't call it directly anymore.

  SRS layer (src/hooks/useSRS.js)
  - replaceStore now hash-checks incoming vs current (both with the
    same theme from localStorage). Equal hashes → return false without
    setStore, writeStore, or revision bump. Belt-and-suspenders against
    a no-op merge.
  - New storeRef synced via useEffect (no render-purity violations).

  Engine (src/hooks/useSyncEngine.js)
  - Removed skipNextPushRef (no longer needed — hash check subsumes it).
  - New lastSyncedHashRef (useRef, not useState — never read during
    render, never triggers re-renders). Tracks "the hash of the state
    we KNOW is synced with Atlas".
  - applyRemote now hashes first; hasChanges=false → records hash and
    returns without touching SRS or theme. Zero React updates, zero
    flicker.
  - pushCurrent short-circuits when currentHash === lastSyncedHashRef.
    The 5-min safety-net and pagehide flush become free no-ops when
    nothing has changed.
  - Mutation watcher (the useEffect on srsRevision + theme) hashes
    current state and skips the debounce entirely when equal to
    lastSyncedHashRef. Catches StrictMode double-invoke, hot-reload,
    and parent re-renders that don't actually mutate the SRS.
  - Bootstrap pull, forcePullNow's no-op path, and linkNewToken/unlink
    all reset/refine lastSyncedHashRef at the right moment.

  Verified in Node
  - Same content, different insertion order → same hash. PASS
  - Same content, different order array → different hash. PASS
  - Theme-only change → different hash. PASS
  - nullTheme === emptyTheme (both hash to same value). PASS
  - cardHash ignores per-device id. PASS
  - 100 hashes × 100 cards = 44 ms (well under one frame at 60 fps).

  Lint clean (React 19 purity / set-state-in-effect / immutability
  rules respected — useEffect-driven refs only, no setState in render),
  build clean (1821 modules, +1 module, ~376 KB JS / ~134 KB CSS;
  +1.3 KB raw for the hash utility).

(prior session in this same chat — sync engine landing)
6b51fc6 docs(handoff): record silent multi-device sync engine (3539a89)
3539a89 feat(sync): silent local-first multi-device sync engine   (prior commit)
6b51fc6 docs(handoff): record silent multi-device sync engine (3539a89)
...
- **Hash collision risk is non-zero but acceptable**: FNV-1a 32-bit gives us ~4 billion distinct hashes. With a 100-card deck the collision probability is astronomically low (~10⁻²⁰ for any two distinct decks), but a collision would mean two distinct states hash identically and one side's mutation is suppressed. Real-world impact is "user makes a change, the engine thinks nothing changed, and the push is skipped until the next mutation". The next mutation always re-hashes, so the worst case is a single missed push — never data loss. Switching to a 64-bit hash (FNV-1a or xxhash) would make collisions practically impossible but would double the per-ref memory cost. Not worth it for a single-user SRS deck.
