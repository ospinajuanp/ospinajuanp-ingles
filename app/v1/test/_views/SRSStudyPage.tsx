...
import type { Grade, SrsCard } from '@/lib/types/srs'
...
export function SRSStudyPage() {
  ...
  const activeCard: SrsCard | null = cursor < queue.length ? (queue[cursor] ?? null) : null

  function handleAdd(payload: { es: string; en: string }): void {
    const created = srs.addCustomSentence(payload)
    if (created) setShowForm(false)
  }

  function handleGrade(cardId: string, grade: Grade): void {
          ...
          <div className="space-y-6">
            ...
            {activeCard ? (
              <section className="rounded-3xl border border-base-300 bg-gradient-to-b from-base-100 to-base-200 p-5 shadow-sm sm:p-8">
                <div className="mb-4 flex items-center justify-between text-xs uppercase tracking-[0.18em] text-base-content/70">
                  <span>
...
interface ModeToggleProps {
  mode: 'study' | 'manage'
  onChange: (next: 'study' | 'manage') => void
}
...
function Stat({ label, value, accent }: StatProps) {
  ...
  return (
    <div className="rounded-2xl border border-base-300 bg-base-100 p-4 shadow-sm">
      <dt className="text-[0.65rem] font-semibold uppercase tracking-[0.18em] text-base-content/70">
        {label}
      </dt>
...
function EmptyState({ dueCount, totalCount, onShowForm, onSwitchToManage }: EmptyStateProps) {
  if (totalCount === 0) {
    return (
