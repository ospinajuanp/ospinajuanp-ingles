...
import type { SrsCard, SrsCustomCard } from '@/lib/types/srs'

const PAGE_SIZE = 10

function cardEs(card: SrsCard): string {
  return card.type === 'custom' ? (card.front?.es ?? '') : (card.infinitivo?.esp ?? '')
}

function cardEn(card: SrsCard): string {
  return card.type === 'custom' ? (card.front?.en ?? '') : (card.infinitivo?.ing ?? '')
}

function formatInterval(card: SrsCard): string {
  if (!card.srs) return '—'
...
export default function DeckManager() {
  ...
  const cards = useMemo<SrsCard[]>(() => srs?.cards ?? [], [srs?.cards])

  const [searchTerm, setSearchTerm] = useState('')
  const [page, setPage] = useState(1)
  const [trackedSearch, setTrackedSearch] = useState(searchTerm)
  const [editingId, setEditingId] = useState<string | null>(null)
  const [deletingCard, setDeletingCard] = useState<SrsCard | null>(null)

  const editingCard = useMemo<SrsCustomCard | null>(
    () =>
      editingId
        ? (cards.find((c) => c.id === editingId && c.type === 'custom') as SrsCustomCard | undefined) ?? null
        : null,
    [editingId, cards],
  )
  ...
  const filtered = useMemo<SrsCard[]>(() => {
...
function buildPageList(page: number, total: number): (number | '…')[] {
  ...
  for (let i = 0; i < list.length; i++) {
    const value = list[i] as number
    withGaps.push(value)
    if (i < list.length - 1 && (list[i + 1] as number) - value > 1) withGaps.push('…')
  }
  return withGaps
}
...
function EditDialog({
  ...
  card: SrsCustomCard | null
  onClose: () => void
  onSave: (payload: { es: string; en: string }) => void
}) {
  ...
  const [trackedId, setTrackedId] = useState<string | null>(card?.id ?? null)
